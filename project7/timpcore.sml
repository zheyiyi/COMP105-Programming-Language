...
(* abstract syntax and values for \timpcore 386d *)
(* definitions of [[exp]] and [[value]] for \timpcore 385d *)
datatype value = NUM   of int
               | ARRAY of value array
(* definitions of [[exp]] and [[value]] for \timpcore 385e *)
datatype exp   = LITERAL of value
               ...
               (* array extensions to \timpcore's abstract syntax 402a *)
               | AAT   of exp * exp
               | APUT  of exp * exp * exp
               | AMAKE of exp * exp
               | ASIZE of exp
(* definition of type [[func]], to represent a \timpcore\ function 386c *)
datatype func = USERDEF   of name list * exp
              | PRIMITIVE of value list -> value
(* definition of [[def]] for \timpcore 386a *)
type userfun = { formals : (name * ty) list, body : exp, returns : ty }
datatype def = VAL    of name * exp
             | EXP    of exp
             | DEFINE of name * userfun
(* definition of [[unit_test]] for \timpcore 386b *)
...
(* definition of [[valueString]] for \timpcore 1281d *)
...
(* definition of [[expString]] for \timpcore 1281a *)
fun expString e =
  let fun bracket s = "(" ^ s ^ ")"
...
(* abstract syntax and values for \timpcore 402b *)
...
(* utility functions on \timpcore\ values 1279a *)
...
(* type checking for \timpcore 392a *)
fun typeof (e, globals, functions, formals) =
  let
 (* function [[ty]], to check the type of an expression, given $\itenvs$ 392b *)
      fun ty (LITERAL v) = INTTY

 (* function [[ty]], to check the type of an expression, given $\itenvs$ 393a *)
        | ty (VAR x) = (find (x, formals) handle NotFound _ => find (x, globals)
                                                                               )

 (* function [[ty]], to check the type of an expression, given $\itenvs$ 393b *)
        | ty (SET (x, e)) =
 ...
 (* function [[ty]], to check the type of an expression, given $\itenvs$ 394a *)
        | ty (IFX (e1, e2, e3)) =
            let val tau1 = ty e1
                val tau2 = ty e2
                val tau3 = ty e3
 ...
 (* function [[ty]], to check the type of an expression, given $\itenvs$ 394c *)
        | ty (BEGIN es) =
 ...
 (* function [[ty]], to check the type of an expression, given $\itenvs$ 395c *)
        | ty (APPLY (f, actuals)) =
             let val actualtypes                     = map ty actuals
                 val FUNTY (formaltypes, resulttype) = find (f, functions)
                 (* definition of [[parameterError]] 396a *)
      ...
      | ty (AAT (a, i))       = (case (ty a, ty i) of
                                  (ARRAYTY t, INTTY) => t
                                  | (ARRAYTY _, _) => 
                                   raise TypeError ("Variable should be of type integer") 
                                  | (_, _) => raise TypeError ("Cannot access property length of non-array variable"))


      | ty (APUT (a, i, e))    = (case (ty a, ty i, ty e) of
                                  (ARRAYTY t1, INTTY, t2) => 
                                   if eqType(t1, t2) then t1
                                  ...
                                  | (_,_,_) => raise TypeError ("Cannot set value of mismatched types"))


      | ty (AMAKE (len, init)) = (case (ty len, ty init) of
                                  (INTTY, t) => ARRAYTY t
                                  | (_, t) => raise TypeError ("Variable should be of type integer"))
...
(* type checking for \timpcore 396b *)
fun elabdef (d, globals, functions) =
    case d
      of (* cases for elaborating definitions in \timpcore 396c *)
           VAL (x, e) =>
         ...
         (* cases for elaborating definitions in \timpcore 397a *)
         | EXP e =>
             let val tau = typeof (e, globals, functions, emptyEnv)
             in  (globals, functions, typeString tau)
             end
         (* cases for elaborating definitions in \timpcore 397b *)
...
(* lexical analysis and parsing for \timpcore, providing [[filexdefs]] and [[stringsxdefs]] 1282a *)
...
(* parsers and [[xdef]] streams for \timpcore 1282b *)
...
(* parsers and [[xdef]] streams for \timpcore 1283b *)
...
(* parsers and [[xdef]] streams for \timpcore 1283c *)
...
(* parsers and [[xdef]] streams for \timpcore 1283d *)
...
val xdeftable = usageParsers
  ...
  (* rows added to \timpcore\ [[xdeftable]] in exercises 1284b *)
...
val xdef =  DEF  <$> deftable 
        ...
        <|> DEF <$> EXP <$> exp
        <?> "definition"
(* parsers and [[xdef]] streams for \timpcore 1284a *)
...
(* evaluation, testing, and the read-eval-print loop for \timpcore 1284c *)
(* definitions of [[eval]] and [[evaldef]] for \timpcore 1284d *)
val unitVal = NUM 1983
(* definitions of [[eval]] and [[evaldef]] for \timpcore 1285a *)
fun eval (e, globals, functions, formals) =
  let fun toBool (NUM 0) = false
      ...
      fun ev (LITERAL n)          = n
        ...
        | ev (APPLY (f, args)) =
            (case find (f, functions)
               of PRIMITIVE p  => p (map ev args)
                | USERDEF func =>
                    (* apply user-defined function [[func]] to [[args]] 1285b *)
        ...
        (* more alternatives for [[ev]] for \timpcore 402c *)
        | ev (AAT (a, i)) =
...
(* definitions of [[eval]] and [[evaldef]] for \timpcore 1286a *)
fun evaldef (d, globals, functions) =
  case d 
    of VAL (x, e) => (* evaluate [[e]] and bind the result to [[x]] 1286b *)
...
val _ = op evaldef : def * value ref env * func env -> value ref env * func env
                                                                        * string
(* definitions of [[basis]] and [[processDef]] for \timpcore 398a *)
...
(* definition of [[testIsGood]] for \timpcore 1279b *)
...
(* shared [[checkTypePasses]] and [[checkTypeErrorPasses]], which call [[ty]] 1280c *)
      ...
      (* definition of [[checkFunctionTypePasses]] 1280a *)
...
(* implementations of \timpcore\ primitives and definition of [[initialBasis]] 399a *)
...
(* utility functions and types for making \timpcore\ primitives 1286e *)
val arithtype = FUNTY ([INTTY, INTTY], INTTY)
(* type declarations for consistency checking *)
val _ = op arithtype : funty
(* utility functions and types for making \timpcore\ primitives 1287a *)
...
val initialBasis =
  let fun addPrim ((name, prim, funty), (tfuns, vfuns)) = 
      ...
      val (tfuns, vfuns)  = foldl addPrim (emptyEnv, emptyEnv)
                            (
                           (* primitive functions for \timpcore\ [[::]] 1286f *)
                             ("+", arithOp op +,   arithtype) :: 
                             ("-", arithOp op -,   arithtype) :: 
                             ("*", arithOp op *,   arithtype) :: 
                             ("/", arithOp op div, arithtype) ::

                           (* primitive functions for \timpcore\ [[::]] 1286g *)
                           ...
                           (* primitive functions for \timpcore\ [[::]] 1287b *)
